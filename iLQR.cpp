...
class iLQR {
   ...
   //输入原有的状态集和控制集，以及计算的k,K,更新新的状态集和控制集
   void forward_pass(vector<vector<double>> X,vector<vector<double>> U,vector<vector<double>> k,vector<vector<vector<double>>> K) {
       ...
       for (int i = 0; i < model.N; ++i) {
           ...
           U_new[i] = m.vectorAdd(m.vectorAdd(U[i],k[i]),m.matToVec(m.matrixMultiply(K[i],m.vecToMat(m.vectorSubtract(X_new[i],X[i])))));
           X_new[i + 1] = model.forward_simulate(X_new[i],U_new[i]);
       }
   }
   //backward_pass过程
   
   void backward_pass(vector<vector<double>> X,vector<vector<double>> U) {
       //此处限制条件应该写在另外一个文件中
       ...
       vector<vector<vector<double>>> l_u = l.get_control_first_derivatives(U,a_high,a_low, delta_bar);
       vector<vector<vector<double>>> l_uu = l.get_control_second_derivatives(U, a_high, a_low, delta_bar);
